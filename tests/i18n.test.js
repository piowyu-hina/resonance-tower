...
for (const file of ['prototype/js/locales/zh-Hant.js', 'prototype/js/i18n.js']) {
...
vm.runInContext("RT_LOCALES.en = { 'header.bag': 'Bag' }; currentLocale = 'en'", context);
assert.equal(vm.runInContext("t('header.bag')", context), 'Bag');
assert.equal(vm.runInContext("t('journal.next')", context), '下一頁');
...
console.log(`i18n tests passed (${markerKeys.length} HTML markers and ${sourceKeys.length} source keys checked)`);
