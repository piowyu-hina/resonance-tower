...
Verified on the user's actual local URL: growth image source is the versioned filename; returned image SHA-256 matches the local approved file; the visible combat action also uses the versioned filename. Screenshot: test-results/wuming-action-live-server.png. No balance or skill behavior changes.
